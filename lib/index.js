...
import { extractAmount, extractClaimNumber, extractSolPublicKey, IsAttemptComment, IsBountyComment, } from "./lib/utils.js";
export default (app) => {
    app.on("pull_request.closed", async (context) => {
        ...
        const transactionDetails = await sendSolToPublicKey(userWallet.privateKey, contributorSolAddress, parseFloat(BountyIssue.bountyAmount));
        // comment to notify user with tx details
        const comment = context.issue({
            issue_number: parseInt(issueNo),
            body: `Thanks to @${contributorName} for winning this bounty of $${BountyIssue.bountyAmount}. Here is the transaction hash: [${transactionDetails}](https://explorer.solana.com/tx/${transactionDetails})!`,
        });
